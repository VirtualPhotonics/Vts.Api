...
{
    internal class SolutionDomainPlotParametersTests
    {
        [Test]
        public void Test_deserialize_forward_solver_data()
        {
            ...
            Assert.That(solutionDomainPlotParameters.XAxis.AxisRange.Start, Is.EqualTo(xAxis.Start));
            Assert.That(solutionDomainPlotParameters.XAxis.AxisRange.Stop, Is.EqualTo(xAxis.Stop));
            Assert.That(solutionDomainPlotParameters.XAxis.AxisRange.Count, Is.EqualTo(xAxis.Count));
            Assert.That(solutionDomainPlotParameters.XAxis.AxisRange.Delta, Is.EqualTo(xAxis.Delta));
            Assert.That(solutionDomainPlotParameters.ForwardSolverType, Is.EqualTo(ForwardSolverType.DistributedPointSourceSDA));
            Assert.That(solutionDomainPlotParameters.InverseSolverType, Is.EqualTo(ForwardSolverType.PointSourceSDA));
            Assert.That(solutionDomainPlotParameters.SolutionDomain, Is.EqualTo(SolutionDomainType.ROfRho));
            Assert.That(solutionDomainPlotParameters.OpticalProperties.Mua, Is.EqualTo(opticalProperties.Mua));
            Assert.That(solutionDomainPlotParameters.OptimizerType, Is.EqualTo(OptimizerType.MPFitLevenbergMarquardt));
            Assert.That(solutionDomainPlotParameters.OptimizationParameters, Is.EqualTo(InverseFitType.MuaMusp));
            Assert.That(solutionDomainPlotParameters.NoiseValue, Is.EqualTo(0));
            Assert.That(solutionDomainPlotParameters.ModelAnalysis, Is.EqualTo(ForwardAnalysisType.R));
            Assert.That(solutionDomainPlotParameters.MeasuredData, Is.Null);
        }
        ...
        public void Test_deserialize_forward_solver_with_wavelength()
        {
            ...
            var solutionDomainPlotParameters = JsonConvert.DeserializeObject<SolutionDomainPlotParameters>(postData);
            Assert.That(solutionDomainPlotParameters.IndependentAxis.Axis, Is.EqualTo(IndependentVariableAxis.Wavelength));
            Assert.That(solutionDomainPlotParameters.IndependentAxis.AxisRange.Start, Is.EqualTo(independentAxis.Start));
            Assert.That(solutionDomainPlotParameters.IndependentAxis.AxisRange.Stop, Is.EqualTo(independentAxis.Stop));
            Assert.That(solutionDomainPlotParameters.IndependentAxis.AxisRange.Count, Is.EqualTo(independentAxis.Count));
            Assert.That(solutionDomainPlotParameters.IndependentAxis.AxisRange.Delta, Is.EqualTo(independentAxis.Delta));
            Assert.That(solutionDomainPlotParameters.XAxis.AxisRange.Start, Is.EqualTo(xAxis.Start));
            Assert.That(solutionDomainPlotParameters.XAxis.AxisRange.Stop, Is.EqualTo(xAxis.Stop));
            Assert.That(solutionDomainPlotParameters.XAxis.AxisRange.Count, Is.EqualTo(xAxis.Count));
            Assert.That(solutionDomainPlotParameters.XAxis.AxisRange.Delta, Is.EqualTo(xAxis.Delta));
            ...
            Assert.That(solutionDomainPlotParameters.InverseSolverType, Is.EqualTo(ForwardSolverType.PointSourceSDA));
            Assert.That(solutionDomainPlotParameters.SolutionDomain, Is.EqualTo(SolutionDomainType.ROfRho));
            Assert.That(solutionDomainPlotParameters.WavelengthOpticalPropertyList[0].Mua, Is.EqualTo(opticalPropertyList[0].Mua));
            Assert.That(solutionDomainPlotParameters.WavelengthOpticalPropertyList[1].Mua, Is.EqualTo(opticalPropertyList[1].Mua));
            Assert.That(solutionDomainPlotParameters.WavelengthOpticalPropertyList[2].Musp, Is.EqualTo(opticalPropertyList[2].Musp));
            Assert.That(solutionDomainPlotParameters.OptimizerType, Is.EqualTo(OptimizerType.MPFitLevenbergMarquardt));
            Assert.That(solutionDomainPlotParameters.OptimizationParameters, Is.EqualTo(InverseFitType.MuaMusp));
            Assert.That(solutionDomainPlotParameters.NoiseValue, Is.EqualTo(0));
            Assert.That(solutionDomainPlotParameters.ModelAnalysis, Is.EqualTo(ForwardAnalysisType.R));
        ...
        public void Test_deserialize_forward_solver_rofrhoandt_with_wavelength()
        {
            ...
            var solutionDomainPlotParameters = JsonConvert.DeserializeObject<SolutionDomainPlotParameters>(postData);
            Assert.That(solutionDomainPlotParameters.SecondIndependentAxis.Axis, Is.EqualTo(IndependentVariableAxis.Time)); 
            Assert.That(solutionDomainPlotParameters.SecondIndependentAxis.AxisValue, Is.EqualTo(0.05));
            Assert.That(solutionDomainPlotParameters.IndependentAxis.Axis, Is.EqualTo(IndependentVariableAxis.Wavelength));
            Assert.That(solutionDomainPlotParameters.IndependentAxis.AxisRange.Start, Is.EqualTo(independentAxis.Start));
            Assert.That(solutionDomainPlotParameters.IndependentAxis.AxisRange.Stop, Is.EqualTo(independentAxis.Stop));
            Assert.That(solutionDomainPlotParameters.IndependentAxis.AxisRange.Count, Is.EqualTo(independentAxis.Count));
        ...
        {
            const string postData = "{\"forwardSolverType\":\"DistributedPointSourceSDA\",\"solutionDomain\":\"ROfFxAndFt\",\"IndependentAxis\":{\"axis\":\"ft\",\"axisValue\":0.05},\"xAxis\":{\"axis\":\"fx\",\"axisRange\":{\"start\":0,\"stop\":0.5,\"count\":51}},\"opticalProperties\":{\"title\":\"Optical Properties\",\"mua\":0.01,\"musp\":1,\"g\":0.8,\"n\":1.4},\"modelAnalysis\":\"dRdMusp\",\"noiseValue\":\"0\"}";
            ...
            Assert.That(solutionDomainPlotParameters.ForwardSolverType, Is.EqualTo(ForwardSolverType.DistributedPointSourceSDA));
            Assert.That(solutionDomainPlotParameters.InverseSolverType, Is.EqualTo(ForwardSolverType.PointSourceSDA));
            Assert.That(solutionDomainPlotParameters.SolutionDomain, Is.EqualTo(SolutionDomainType.ROfFxAndFt));
            Assert.That(solutionDomainPlotParameters.OpticalProperties.Mua, Is.EqualTo(opticalProperties.Mua));
            Assert.That(solutionDomainPlotParameters.OptimizerType, Is.EqualTo(OptimizerType.MPFitLevenbergMarquardt));
            Assert.That(solutionDomainPlotParameters.OptimizationParameters, Is.EqualTo(InverseFitType.MuaMusp));
            Assert.That(solutionDomainPlotParameters.NoiseValue, Is.EqualTo(0));
            Assert.That(solutionDomainPlotParameters.ModelAnalysis, Is.EqualTo(ForwardAnalysisType.dRdMusp));
            Assert.That(solutionDomainPlotParameters.MeasuredData, Is.Null);
        }
        ...
        public void Test_deserialize_inverse_solver_data()
        {
            ...
            Assert.That(solutionDomainPlotParameters.XAxis.AxisRange.Count, Is.EqualTo(xAxis.Count));
            Assert.That(solutionDomainPlotParameters.XAxis.AxisRange.Delta, Is.EqualTo(xAxis.Delta));
            Assert.That(solutionDomainPlotParameters.ForwardSolverType, Is.EqualTo(ForwardSolverType.PointSourceSDA));
            Assert.That(solutionDomainPlotParameters.InverseSolverType, Is.EqualTo(ForwardSolverType.PointSourceSDA));
            Assert.That(solutionDomainPlotParameters.SolutionDomain, Is.EqualTo(SolutionDomainType.ROfRho));
            Assert.That(solutionDomainPlotParameters.OpticalProperties.Mua, Is.EqualTo(opticalProperties.Mua));
            Assert.That(solutionDomainPlotParameters.OptimizerType, Is.EqualTo(OptimizerType.MPFitLevenbergMarquardt));
            ...
            Assert.That(solutionDomainPlotParameters.NoiseValue, Is.EqualTo(0));
            Assert.That(solutionDomainPlotParameters.ModelAnalysis, Is.EqualTo(ForwardAnalysisType.R));
            Assert.That(solutionDomainPlotParameters.MeasuredData[3][0], Is.EqualTo(measuredData[3][0]));
            Assert.That(solutionDomainPlotParameters.MeasuredData[7][1], Is.EqualTo(measuredData[7][1]));
            Assert.That(solutionDomainPlotParameters.MeasuredData[5][0], Is.EqualTo(measuredData[5][0]));
        }
    }
}
